    ...
    <table cellspacing="0">
      <tr v-for="(row, rowIndex) in game" v-bind:key="rowIndex">
        <td
          ...
          v-on:click="click($event, rowIndex, columnIndex)"
        >
          <div id="box" class="dnone">{{ game[rowIndex][columnIndex] }}</div>
        </td>
      </tr>
    </table>
...
export default {
  ...
  methods: {
    ...
    click: function click($event, x, y) {
      // Check if cell is already clicked.
      if (!$event.target.classList.contains("dnone")) {
        return false;
      }
